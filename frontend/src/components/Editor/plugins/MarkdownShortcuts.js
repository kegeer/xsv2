  ...
  return {
    ...
    onSpace(ev, change) {
      ...
      for (const key of inlineShortcuts) {
        ...
        // only add tags if they have spaces around them or the tag is beginning or the
        // end of the block
        ...
        if (inlineTags.length > 1) {
          ...
          return change
            ...
            .moveOffsetsTo(firstCodeTagIndex, lastCodeTagIndex - shortcut.length)
            .addMark(mark)
            .collapseToEnd()
            .removeMark(mark);
        }
    ...
    onBackspace(ev, change) {
      ...
      if (selection.isCollapsed) {
        ...
        if (codeMarksAtCursor.size > 0) {
          ...
          const charsInCodeBlock = textNode
            .characters
    ...
    onEnter(ev, change) {
      ...
      // Hitting enter while an image is selected should jump caret below and insert a
      // new paragraph
      ...
      // Hitting enter in a heading or blockquote will split the node at that point
      // and make the new node a paragraph
