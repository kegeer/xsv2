...
class ProjectNew extends Component {
  ...
  handleSubmit = async (ev) => {
    ...
    const projectId = Object.values(project.projects)[0].id;
    this.setState({
      isSaving: false
    });
    this.props.clearActiveModal();
    this.props.history.push(projectId ? `/projects/${projectId}` : '/dashboard');
  };
  ...
  handleOpenChange = (ev) => {
    this.setState({
      publicStatus: ev.target.checked
    });
  };
  handleColor = (color) => {
    this.setState({ color });
  ...
  render() {
    const { name, summary, publicStatus, color, isSaving } = this.state;
    return (
